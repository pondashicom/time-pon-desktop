(function () {
    ...
    function refreshUIFromState(payload) {
        ...
        if (payload.timer) {
            currentTimer = payload.timer;
            elTime.textContent = payload.timer.timeText || '00:00:00';

            if (!payload.timer.running) {
                setStateText('停止');
            } else if (payload.timer.paused) {
                setStateText('一時停止');
            } else {
                setStateText('実行中');
            }
        ...
        if (payload.overlay) {
            currentOverlay = payload.overlay;
    ...
    window.timepon.onTimerTick((t) => {
        if (!t) return;
        currentTimer = t;
        elTime.textContent = t.timeText || '00:00:00';
